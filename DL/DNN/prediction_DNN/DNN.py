...
class DNN:
    ...
    def __init__(self):
        print("__init__");
    def __init__(self,num_epochs = 20, batch_size=30, filterNum=1, bias=True, activation='relu', output_layer_activation='linear', output_loss = 'mse'):
    ...
    def build_DNN(self, pVal, n_outputs ,coeff=0):
        ...
        dense1_d = Dropout(0.05)(dense1) # Dropout here 0.05
        dense2 = Dense(pVal, activation=self.activation, use_bias=self.bias, kernel_initializer='glorot_normal', kernel_regularizer=regularizers.l1(coeff))(dense1_d);
        self.show_layer_info('Dense', dense2);
        dense2_d = Dropout(0.05)(dense2)# Dropout here 0.05
        outputs = Dense(n_outputs, activation=self.output_layer_activation, kernel_initializer='glorot_normal')(dense2_d)#
